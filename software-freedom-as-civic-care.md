...
The second condition is _sub-quadratic attention_. The reason fine-tuning has been unaffordable on a laptop for so long is that classical attention is quadratic: every token attends to every other token, so the more context the model holds, the more compute it has to throw at every step. Sub-quadratic, near-linear attention mechanisms are beginning to roll out beyond research labs into real deployments. As they mature, communities will be able to run _interaction models_ that learn from being observers in a real social system, aligning to the loop of the community itself rather than to a curated dataset some vendor labelled. That direction of travel is opening up now.
