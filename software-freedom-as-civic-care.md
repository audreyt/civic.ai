...
Two technical conditions have to hold for this to work locally, and both have recently come into reach. The first is _directional steering_ — the ability for a community to say "these are good answers, these are not" and have the model shift within minutes rather than after the vendor's six-month release cycle. We do this with a technique called a _hypernetwork_: once the hypernetwork itself has been trained, it can take a document (a code of conduct, a long codebase, a set of good/bad examples) and produce low-rank adapters — LoRA — for an existing base model in a single forward pass, in seconds, on off-the-shelf laptop hardware. You only get to do this if you own the inference loop. If you are renting somebody else's inference engine, the only steering anyone can do is the steering the vendor permits.

The second condition is _sub-quadratic attention_. The reason fine-tuning has been unaffordable on a laptop for so long is that classical attention is quadratic: every token attends to every other token, so the more context the model holds, the more compute it has to throw at every step. Sub-quadratic attention mechanisms — closer to linear in the context length — are beginning to roll out beyond research labs into real deployments. As they mature, communities will be able to run _interaction models_ that learn from being observers in a real social system, aligning to the loop of the community itself rather than to a curated dataset some vendor labelled. That direction of travel is opening up now.
