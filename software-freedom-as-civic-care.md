...
I crossed from forkable tools, at fifteen, into state power as Taiwan's first Digital Minister when I turned thirty-five. I also worked, in between, for six years with proprietary AI — namely Siri. And then I came back to this community, convinced that forkability is civic care: not a developer's hobbyhorse, not a licence preference, but a civic instrument.
...
Stallman set up the FSF in 1985; the Free Software Definition followed in 1986, and the numbered freedoms 0 through 3 took their present form a few years later. Many of you can recite them. I want to read them again, as muscles rather than as licences.
...
Of course, @antirez knows about good-enough ancestors. He shipped Redis under BSD-3-Clause for roughly fifteen years. When Redis Inc. moved future Redis releases to RSALv2/SSPLv1 in March 2024, that prompted the community to fork Valkey from the last BSD release; in 2025, the company added AGPLv3 — the GNU Affero General Public License v3 — as a third licensing option, the strong copyleft for the cloud and AI era. The decision made years earlier — to ship under a licence the community could not have revoked — was the ancestor decision: good enough, not perfect.
...
Two technical conditions have to hold for this to work locally, and both have recently lined up. The first is _directional steering_ — the ability for a community to say "these are good answers, these are not" and have the model shift within minutes rather than after the vendor's six-month release cycle. We do this with a technique called a _hypernetwork_: a small network that takes a document (a code of conduct, a long codebase, a set of good/bad examples) and produces low-rank adapters — LoRA — on an existing base model, in seconds, on off-the-shelf laptop hardware. You only get to do this if you own the inference loop. If you are renting somebody else's inference engine, the only steering anyone can do is the steering the vendor permits.

The second condition is _sub-quadratic attention_. The reason fine-tuning was unaffordable on a laptop for so long is that classical attention is quadratic: every token attends to every other token, so the more context the model holds, the more compute it has to throw at every step. Sub-quadratic — almost linear — attention mechanisms are now rolling out beyond labs into real deployments. With near-linear training, communities can run _interaction models_ that learn from being observers in a real social system, aligning to the loop of the community itself rather than to a curated dataset some vendor labelled. That shift is maybe less than a year away.
...
Parliament passed it within months. In later reporting, the Ministry of Digital Affairs said enforcement was associated with category-specific drops: ninety-six per cent for investment-scam ads, and ninety-four per cent for identity-impersonation-scam ads — not all scams as a whole, and not deepfake ads as such. Reuters separately reported that Meta's internal documents showed persistent scam-ad revenue incentives, and that Taiwan-style advertiser-verification rules were being watched by regulators elsewhere, including Japan, which is now considering a similar system.
...
On the 16th of October 2022, Professor Tim Davis at Texas A&M showed GitHub Copilot outputting `cs_transpose` from CSparse — a component of SuiteSparse — with matching variable names and comments. CSparse is licensed under LGPL-2.1-or-later; the Copilot output carried neither attribution nor licence.
...
This whole argument runs from Stallman's four freedoms and the GPL, through Debian's Social Contract and the Open Source Definition, Apache-style permissive governance, Linux-scale maintenance, and the broader legal lineage — Wendy Seltzer and others on the policy side, Karen Sandler and the Software Freedom Conservancy on the courtroom side — who together kept those freedoms enforceable. When the SFC sued Vizio in 2021 for shipping smart TVs running Linux without releasing the source, their precise legal claim was third-party-beneficiary rights for recipients of GPL/LGPL software: the purchaser of the TV is a beneficiary of the licence, not a stranger to it. "A covenant with the public" is the moral gloss; the legal theory is what makes it bite. The right to inspect the code in the hardware in your living room is a right held by you, not by a corporate licensee. That argument is still live. It matters more for AI than it ever did for TVs.
