...
- **最小權限。** 使用能完成工作的最簡單機制；每多一個元件，就多一處可能讓失敗、混淆或攻擊進入的地方。
...
仁工智慧進入這些場域時，是一名數位學徒，而不是替代者：它先在影子模式中觀察、記錄決策軌跡，學會這間工坊、這個病房、這個社群的內隱知識，然後才被信任去提出建議，更不用說採取行動。而在這裡，培育者手上始終握著三樣東西：**方向盤**（執行始終由人指揮）、**煞車**（一個醒目的控制，能立刻停下機器，線路直連、定期測試），以及**維修手冊**（可追溯哪些規則被觸發、哪些輸入引導了輸出，以及模型故障時由誰修復）。可攜性與退出權仍然重要，但它們屬於 Pack 5 的團結力工作；此處較窄的測試，是在任何契約條款生效以前，可檢查性與覆寫權是否真的在房間裡管用。
...
- **安全即關懷義務。** 安全屬於勝任力，因為有能力的智慧體會透過它取得的權限傷人。當仁工智慧系統能讀取檔案、改動紀錄，或代表某人行動時，過寬的權限就是在花用被關懷者的信任，卻沒有回來詢問。
    - 最小權限、嚴格沙盒、已驗證的輸入，以及經測試的出口控制，定義了學徒可以碰觸什麼、一道混淆指令能傳多遠，以及誰能在入侵變成關懷失敗之前停下它。
